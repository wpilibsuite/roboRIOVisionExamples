...
2. The LabVIEW example code calculates 6 separate ratios for the target. 
...
larger of the two (e.g. 1L = 1st contour left edge) and 2 with a letter is the 2nd contour. (H=Height, L = Left, T = Top, B = Bottom, W = Width, X = X center, Y = Y center)


	Bounding Box (rect which contains both contours) Height = 2* width: BB_Height / (2*BBWidth)
	1st contour width = ~1/4 target width: 1W*4/ BB_Width
	Top edge difference: ((1T - 2T) / BB_Height) + 1
	Spacing between left edges = 3/4 target width: (1X - 2X)/ 2*BB_Width
	Width Ratio: 1W/2W
	Height Ratio: 1H/2H
